...
def find_noontide(df, row_name):
    """
    Find the column with the highest value in the row, and return the column, and the column -3 position.
    If the column -3 is not available, then return the column with the second highest value, and apply the same logic.
    If still not available, use the third highest value.
    ...
    # Function to get the desired column and its -3 position
    def get_columns(row):
        column_n = row.idxmax()
        col_idx = list(df.columns).index(column_n)

        # Check if the desired -3 position is valid
        if col_idx - 2 >= 0:
            column_n_minus_3 = df.columns[col_idx - 2]
            return [column_n, column_n_minus_3]
        else:
            # Invalidate the current highest value and try again
            row[column_n] = np.nan
            return None

    columns = get_columns(row)

    # Try second highest if the first choice is invalid
    if columns is None:
        columns = get_columns(row)
    
    # Try third highest if the second choice is invalid
    if columns is None:
        columns = get_columns(row)

    if columns is None:
        raise Exception("Unable to find a suitable -3 position column.")
...
def no_repeat_fc(df, noontide):
    ...
    # Get the positions of noontide[0] and noontide[1]
    col_pos_0 = df.columns.get_loc(noontide[1])
    col_pos_1 = df.columns.get_loc(noontide[1]) + 1
    
    # Adjust positions if they are less than 0
    col_pos_0 = max(0, col_pos_0)
    col_pos_1 = max(0, col_pos_1)
    
    # Get the column names for the adjusted positions
    col_name_0 = df.columns[col_pos_0]
    col_name_1 = df.columns[col_pos_1]
    
    # Compute log2FoldChange
    new_df['log2FoldChange'] = new_df[col_name_0] / new_df[col_name_1]
    # new_df['log2FoldChange'] = new_df['log2FoldChange'].apply(lambda x: math.log2(x if x > 0 else np.finfo(float).eps))
